...
[Source code](./handout/Bug Bounty 4)
...
In the source code the name variable is 48 bytes but `fgets` function expect us to enter 64 bytes. That mean we can overwrite `ptr` and `value` because they are next to name and next to each other  `̀[name][ptr][value][return]` 
...
payload += p64(PRINT_FLAG) # overwrite value with "print_flag" address
