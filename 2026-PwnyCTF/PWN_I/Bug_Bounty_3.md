...
Why did we send 56 A's ? Because after `name` who is 48 bytes there is the  `saved RBP` value who is 8 bytes between `name` and the return address.  
The stack will look like this `[name][saved_RBP][return]`. So we need to overwrite the `saved RBP` with 8 bytes before we can overwrite the return address `48 + 8 = 54`  
